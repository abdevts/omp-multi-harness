...
export function processFailed(agent: AgentName, exitCode: number | null, stderrTail?: string): AgentError {
	return new AgentError({
		code: "PROCESS_FAILED",
		agent,
		message: `${DISPLAY[agent]} exited with code ${exitCode ?? "null"}.${stderrTail ? ` Last output: ${stderrTail}` : ""}`,
		exitCode,
		stderrTail,
	});
}
...
export function fromSpawnError(agent: AgentName, executable: string, err: NodeJS.ErrnoException): AgentError {
	...
	return new AgentError({ code: "PROCESS_FAILED", agent, message: `Failed to start ${executable}: ${err.message}`, cause: err });
}
